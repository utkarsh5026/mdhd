...
const CompletionAnimation: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  useEffect(() => {
...
const WordProgress: React.FC<WordProgressProps> = ({ sections, currentIndex, readSections }) => {
  const [hasCompletedOnce, setHasCompletedOnce] = useState(false);
  const [showCompletion, setShowCompletion] = useState(false);

  const { totalWords, wordsRead, percentage, isCurrentSectionRead, completedSections } =
    useMemo(() => {
  ...
  return (
    <div className="relative rounded-2xl border-4">
      <AnimatePresence>
        {showCompletion && <CompletionAnimation onComplete={handleCompletionEnd} />}
      </AnimatePresence>
        ...
        >
          <motion.span
            ...
            animate={{
              color:
                percentage === 100
                  ? ['hsl(var(--primary))', 'hsl(var(--primary) / 0.9)', 'hsl(var(--primary))']
                  : 'hsl(var(--primary))',
            }}
...
export const MobileProgressIndicator: React.FC<MobileProgressIndicatorProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <WordProgress sections={sections} currentIndex={currentIndex} readSections={readSections} />
      ...
      <div className="flex justify-between items-center">
        <div className="flex flex-col gap-1">
          <span className="text-sm font-semibold text-foreground">Section {currentIndex + 1}</span>
          <span className="text-xs text-mutedForeground">of {total} sections</span>
        </div>
          ...
          >
            <motion.div
              ...
              animate={{
                ...
                color:
                  percentage === 100
                    ? ['hsl(var(--primary))', 'hsl(var(--primary) / 0.8)', 'hsl(var(--primary))']
                    : 'hsl(var(--primary))',
              }}
              transition={{ duration: 0.3 }}
            >
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={cn(
                          ...
                          isActive ? 'bg-primary' : isCompleted ? 'bg-primary/80' : 'bg-muted'
                        )}
                      />
