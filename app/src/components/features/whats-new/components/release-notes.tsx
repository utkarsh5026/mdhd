 ...
 *
 * Hairline rules and type weight carry the structure, so this reads like the
 * app's own writing rather than a product launch page. Accent colour is spent
 * in exactly three places — the version, the highlight icons, and the dismiss
 * button — which keeps it lively without tinting every surface.
 *
...
const ReleaseNotes: React.FC<ReleaseNotesProps> = ({
  ...
  <div className={cn('flex flex-col gap-5', className)}>
    ...
    <p className="flex items-baseline gap-2 text-[11px] text-muted-foreground/40 select-none">
      ...
      <span className="font-medium tabular-nums text-primary">{release.version}</span>
    ...
    <div className="flex justify-end">
      <button
        ...
        onClick={onDismiss}
        className={cn(
          'h-8 rounded-lg bg-primary px-4 text-xs font-medium text-primary-foreground',
