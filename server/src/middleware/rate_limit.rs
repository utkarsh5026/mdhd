...
//!
//! Each client is keyed by the TCP peer address from [`axum::extract::ConnectInfo`]. The app must
//! be served with [`axum::Router::into_make_service_with_connect_info`] so that address is
//! populated.
//!
...
/// Returns a [`GovernorLayer`] that rate-limits by peer IP and maps quota exhaustion to **429**.
///
